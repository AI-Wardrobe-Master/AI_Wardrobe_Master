...
class OutfitRecommendationAgent:
    ...
    def run_chat(
        ...
        *,
        ...
        """Runs one conversational outfit Agent turn.
    ...
    def _load_conversation_context(
        ...
        *,
        ...
        if self.conversation_store is None:
            return _empty_conversation_context(
                user_id,
                conversation_id=conversation_id,
            )
...
def _recommendation_request_from_chat(
    ...
    """Converts a chat request into the core recommendation request shape.

    Args:
        request: Chat request that may include a conversation id.

    Returns:
        Recommendation request without chat-only fields.
    """
    ...
    return OutfitRecommendationRequest.model_validate(payload)


def _assistant_message_from_final(final: dict[str, Any]) -> str:
    """Extracts the user-facing assistant message from an Agent result.

    Args:
        final: Final Agent response payload.

    Returns:
        Assistant text suitable for a chat bubble.
    """
    raw = final.get("rawModelOutput")
    if isinstance(raw, dict):
        user_message = raw.get("userMessage")
        if isinstance(user_message, str) and user_message.strip():
            return user_message.strip()

    reason = final.get("recommendationReason")
...
def _empty_conversation_context(
    user_id: UUID,
    conversation_id: str | None = None,
) -> dict[str, Any]:
